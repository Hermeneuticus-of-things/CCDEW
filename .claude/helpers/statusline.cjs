...
function generateStatusline() {
  ...
  // CodeBurn real: cite\u0219te cache-ul din codeburn CLI
  try {
    const cachePath = path.join(CWD, '.claude-flow', 'data', 'codeburn-cache.json');
    if (fs.existsSync(cachePath)) {
      const burn = readJSON(cachePath);
      if (burn && burn.today_cost !== undefined) {
        const burnBadge = burn.today_cost > 10 ? c.brightRed : burn.today_cost > 5 ? c.brightYellow : c.brightGreen;
        header += '  ' + c.dim + '\u2502' + c.reset + '  ' + burnBadge + '\ud83d\udd25 $' + burn.today_cost.toFixed(2) + c.reset;
      }
    }
  } catch { /* non-fatal */ }
  lines.push(header);
