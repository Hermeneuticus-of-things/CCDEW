# {{NUME_PROIECT}}/doc/INDEX.md — Master Index Detalii Proiect

> **Scop:** Router detalii research. Citit on-demand pe baza task-ului.
> **Parent:** `../CLAUDE.md` (slim)

## Cand citesc ce

| Task | Fisier | Continut principal |
|---|---|---|
| Metodologie si design | `METHODOLOGY.md` | Abordare, criterii selectie, validare |
| Surse si bibliografie | `SOURCES.md` | Lista surse, credibilitate, relevanta |
| Stare analiza + concluzii | `FINDINGS.md` | Rezultate intermediare si finale |

## Reguli cross-project (nu duplicate aici)

| Categorie | Locatie |
|---|---|
| Session start / 3 reguli critice | `_SETTINGS/RULES/session_start.md` |
| Format CHANGELOG | `_SETTINGS/RULES/changelog.md` |
| Best practices cross-project | `D:\Cloude Code\BEST_PRACTICES.md` |
| Best practices {{NUME_PROIECT}} | `../BEST_PRACTICES.md` |

## Fisiere operationale in root proiect

| Fisier | Scop |
|---|---|
| `CLAUDE.md` | Quick-context slim |
| `BEST_PRACTICES.md` | Lectii invatate specific |
| `CHANGELOG.md` | Jurnal cerinte |
| `TODO.md` | Task-uri active |
