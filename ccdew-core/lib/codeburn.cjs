...
const { isEnabled } = require('./flags.cjs');
const { findExecutable } = require('./platform.cjs');
const nativeEngine = require('./codeburn-engine.cjs');
...
function fetchRealStatus() {
  ...
  if (!bin) return null;
  try {
    const { isSafeBinaryPath } = require('./path-safe.cjs');
    if (!isSafeBinaryPath(bin)) {
      try { require('./error-log.cjs').logError('codeburn.unsafe_path', new Error('shell metachar in bin path'), { bin }); } catch {}
      return null;
    }
