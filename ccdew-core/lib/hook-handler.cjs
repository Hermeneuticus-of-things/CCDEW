...
const fs = require('fs');
const { isEnabled, load: loadFlags } = require('./flags.cjs');
const { findPython } = require('./platform.cjs');
...
async function main() {
  ...
  const handlers = {
    ...
    'auto-audit': () => {
      ...
      } catch (e) { try { require('./error-log.cjs').logError('auto-audit', e); } catch {} }
    },
    ...
    'burn': () => {
      ...
      const t = codeburn.totals();
      if (t.source === 'unavailable') {
        console.log('[CODEBURN] ' + require('./strings.cjs').t('codeburn.unavailable'));
