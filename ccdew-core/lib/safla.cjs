...
const { isValidNodeId, asString, clampNumber } = require('./validate.cjs');
...
function load() {
  ...
  } catch (e) { try { require('./error-log.cjs').logError('safla.load', e); } catch {} }
  return emptyState();
}
...
function recordOutcome(nodeId, success, task) {
  ...
  try {
    ...
    try { require('./error-log.cjs').logError('safla.recordOutcome.lock', e); } catch {}
  }
}
